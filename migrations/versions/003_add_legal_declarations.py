...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE ismodified AS ENUM ('yes', 'no')")
    ...
    op.create_table(
        ...
        sa.Column("is_modified", sa.Enum(name="ismodified"), nullable=False, default="no", comment="是否修改"),
        sa.Column("usage_type", sa.Enum(name="usagetype"), nullable=False, comment="使用方式"),
...
def downgrade() -> None:
    ...
    op.execute("DROP TYPE IF EXISTS ismodified")
